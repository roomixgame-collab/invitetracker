...
async function credit(member, creditInfo) {
  ...
  storage.setAttribution(member.id, inviter.id);
...
function processLeaves() {
  ...
  for (const pending of pendingLeaves) {
    ...
    const inviterId = storage.getAttribution(leaverId);
    ...
    const count = storage.decrement(inviterId);
    storage.clearAttribution(leaverId);
