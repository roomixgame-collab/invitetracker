...
function reportNoInviter(pending) {
  const member = pending.member;
  const waited = Math.round((Date.now() - pending.at) / 1000);
  const message =
    `O usuário <@${member.id}> entrou no servidor pelo convite original ` +
    `${config.originalInviteUrl} e nenhum convidador foi identificado ` +
    `(nenhum invite ganhou uso em ${waited}s). Nenhum convite foi creditado.`;

  console.log(`[INVITE] RESULTADO: ${message}`);
  sendLog(member.guild, message).catch((err) =>
    console.log("[INVITE] Erro ao registrar entrada sem convidador:", err.message)
  );
}
