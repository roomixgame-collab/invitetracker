...
async function registerCommands() {
  for (const guild of client.guilds.cache.values()) {
    try {
      ...
      console.log(`[INVITE] Comando /invites registrado em ${guild.name} (${guild.id}).`);
    } catch (err) {
      console.log(
        `[INVITE] Falha ao registrar /invites em ${guild.id}:`,
        err.message
      );
    }
  }
...
client.on("guildMemberRemove", (member) => {
  ...
  setTimeout(() => attribution.processLeaves(), config.joinSettleMs);
});
